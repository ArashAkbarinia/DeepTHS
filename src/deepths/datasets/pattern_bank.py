...
__all__ = ['wave', 'herringbone', 'diamond', 'line', 'grid', 'brick', 'filled', 'empty']
...
def wave(img_size, height, gap, peak=1, length=1):
    ...
    return _repeat_to_img_size(wave_img, img_size)
...
def herringbone(img_size, height, peak=1, length=1):
    ...
    herringbone_img[:, 0] = 1
    return _repeat_to_img_size(herringbone_img, img_size)
...
def grid(img_size, gaps, thicknesses=(1, 1)):
    ...
    grid_img[:, 0:thicknesses[0]] = 1
    return _repeat_to_img_size(grid_img, img_size)
